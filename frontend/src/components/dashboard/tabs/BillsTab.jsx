...
export default function BillsTab({ userWallet }) {
  ...
  // ── helpers ───────────────────────────────────────────────────────────────
  const fmtDate = (ts) => ts ? new Date(ts * 1000).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : '—'
  const statusColor = (s) => {
    const v = (s || '').toLowerCase()
    if (v === 'paid') return 'bg-green-100 text-green-700'
    if (v === 'expired' || v === 'cancelled') return 'bg-red-100 text-red-700'
    return 'bg-amber-100 text-amber-700'
  }

  const pending      = bills.filter(b => b.status === 'Pending')
  const paid         = bills.filter(b => b.status === 'Paid')
  const totalPending = pending.reduce((s, b) => s + Number(b.amountUSD), 0).toFixed(2)
  const totalPaid    = paid.reduce((s, b) => s + Number(b.amountUSD), 0).toFixed(2)
        ...
        <>
          ...
          <div className="space-y-3">
            ...
            {bills.length > 0 ? bills.map((bill, idx) => {
              ...
              return (
                <div key={bill.paymentId || idx} className="rounded-xl border border-border bg-background p-4 space-y-4">
                  ...
                  {/* Smart route section — only for pending, unpaid bills */}
